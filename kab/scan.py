...
with open('kab/clf.pickle', 'rb') as f:
    clf = pickle.load(f)

def lr_ocr(a: np.ndarray) -> str:
    i, = clf.predict([a])
...
def machine_numbers() -> tuple:
    hok = list(range(734, 750))
    kab = list(range(828, 840))
    return hok, kab

def scan_oneday(date: str, hall='kamisato') -> None:
    ...
    print(f'except: {len(numbers)} {sign} dic: {len(dic)}')
    ...
    d |= {'hokto': {str(n): dic[n] for n in hok}}
    d |= {'kaban': {str(n): dic[n] for n in kab}}
